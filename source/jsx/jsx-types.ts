...
/**
 * The runtime type of an JSX expression like `<div/>`, i.e., a function returning the rendered HTML
 * string in the simplest case. Since components can be `async`, however, a JSX expression can
 * alternatively return a `Promise<string>` of the HTML that will eventually be rendered. Moreover,
 * an expression can return either `null` or `Promise<null>` to not render anything at all.
 */
...
/**
 * The actual JSX element type required for TypeScript to successfully type-check asynchronous
 * components. The runtime type of a JSX expression is always `JsxExpression`. The
 * `Promise<JsxExpression>` variant is what additionally allows components to be `async`.
 */
export type JsxElement = null | JsxExpression | Promise<JsxExpression | null>;

/**
 * Denotes the name and type of the `children` prop that the TypeScript uses to determine whether
 * JSX children are allowed.
 */
...
export type JsxComponent<TProps = {}> =
 ...
 */
export function toJsxExpression(expression: () => ReturnType<JsxExpression>): JsxExpression {
    (expression as any)[jsxExpressionSymbol] = null;
    return expression as JsxExpression;
}

/**
 * Checks whether the given value is a JSX expression and can thus safely be rendered as a JSX
 * child.
 */
...
export namespace JSX {
    ...
    export interface AriaAttributes {
         ...
         * user's intended value for an input and specifies how predictions would be presented if
         * they are made.
         */
        ...
        /**
         * Indicates keyboard shortcuts that an author has implemented to activate or give focus to
         * an element.
         */
         ...
         * contextual parent/child relationship between DOM elements where the DOM hierarchy cannot
         * be used to represent the relationship.
         *
         * @see aria-controls.
         */
        "aria-owns"?: string;
         ...
         * when the control has no value. A hint could be a sample value or a brief description of
         * the expected format.
         */
